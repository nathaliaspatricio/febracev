...
urlpatterns = patterns('',
    ...
    url(r'^(?P<project_slug>[a-z0-9-_]+)/(?P<year>\d{4})/(?P<month>\w{3})/(?P<day>\d{1,2})/(?P<slug>[-\w]+)/edit/$',
        view=blog_views.edit_post,
        name='blog_edit_post_slug'),    

    url(r'^(?P<project_slug>[a-z0-9-_]+)/(?P<year>\d{4})/(?P<month>\w{3})/(?P<day>\d{1,2})/(?P<slug>[-\w]+)/$',
        view=blog_views.post_detail,
        name='blog_detail_slug'),

    url(r'^(?P<project_slug>[a-z0-9-_]+)/(?P<year>\d{4})/(?P<month>\w{3})/(?P<day>\d{1,2})/$',
        view=blog_views.post_archive_day,
        name='blog_archive_day_slug'),

    url(r'^(?P<project_slug>[a-z0-9-_]+)/(?P<year>\d{4})/(?P<month>\w{3})/$',
        view=blog_views.post_archive_month,
        name='blog_archive_month_slug'),

    url(r'^(?P<project_slug>[a-z0-9-_]+)/(?P<year>\d{4})/$',
        view=blog_views.post_archive_year,
        name='blog_archive_year_slug'),
    ...
    url(r'^(?P<project_slug>[a-z0-9-_]+)/$',
        view=blog_views.post_list,
        name='blog_index_slug'),

    url(r'^(?P<project_slug>[a-z0-9-_]+)/add/$',
        view=blog_views.add_post,
        name='blog_add_post_slug'),
